...
const MyBookingsPage = () => {
    ...
    /**
     * Tính toán refund percent theo chính sách
     */
    ...
    const handleCancelPaidBooking = (booking) => {
        ...
        if (refund_percent === 0) {
            // ❌ HỦY MUỘN - KHÔNG HOÀN TIỀN
            const message = 
                `⚠️ CẢNH BÁO: HỦY MUỘN!\n\n` +
                `Tour này còn ${daysUntilDeparture} ngày nữa sẽ khởi hành.\n` +
                `Theo chính sách, bạn sẽ KHÔNG được hoàn tiền.\n\n` +
                `📋 Thông tin booking:\n` +
                `- Tour: ${booking.tour_name || booking.destination}\n` +
                `- Ngày khởi hành: ${new Date(booking.departure_date).toLocaleDateString('vi-VN')}\n` +
                `- Tổng tiền đã thanh toán: ${booking.final_amount.toLocaleString('vi-VN')} VNĐ\n\n` +
            ...
            openConfirmationModal(
                ...
                message,
                'Đồng ý hủy',
                () => handleConfirmRefundNoRefund(booking.id)
            );
        } else {
            // CÓ HOÀN TIỀN - MỞ MODAL CHỌN PHƯƠNG THỨC
    ...
    /**
     * Xử lý hủy KHÔNG hoàn tiền (<3 ngày)
     */
    const handleConfirmRefundNoRefund = async (bookingId) => {
        try {
            setSubmittingRefund(true);

            const result = await bookingService.cancelPaidBooking(bookingId, {
                refund_method: 'credit' // Backend sẽ tự set = 'none'
            });

            if (result.success) {
                toast.warning(
                    result.message || 'Tour đã được hủy. Không được hoàn tiền do hủy muộn.',
                    { autoClose: 5000 }
                );
    ...
    /**
     * Xác nhận hủy với phương thức hoàn tiền (từ RefundMethodModal)
     */
    const handleConfirmRefund = async (refundMethod) => {
        ...
        try {
            setSubmittingRefund(true);
            setShowRefundModal(false); // Đóng modal trước

            // GỌI API - CHỈ GỬI refund_method
            ...
            if (result.success) {
                if (result.requiresApproval) {
                    ...
                    // ✅ HỦY NGAY (không hoàn tiền)
                    toast.warning(
                        result.message || 'Tour đã được hủy',
                        { autoClose: 5000 }
                    );
                }
            ...
            console.error('[handleConfirmRefund] Error:', err);
            toast.error(err.message || 'Không thể hủy booking');
        } finally {
            setSubmittingRefund(false);
        }
                ...
                {bookings.length === 0 ? (
                    ...
                    <div className="v2-bookings-list">
                        {bookings.map(booking => {
                            ...
                            // THÊM ĐIỀU KIỆN CHO pending_cancellation
                            ...
                            return (
                                <div key={booking.id} className={`v2-booking-card status-${booking.status}`}>
                                    ...
                                    <div className="v2-booking-card-body">
                                        ...
                                        <div className="v2-booking-info-extra">
                                            <p className="v2-booking-id">
                                                <i className="fas fa-ticket-alt"></i> 
                                                <strong>Mã booking:</strong> 
                                                <span className="booking-id-value">#{booking.id}</span>
                                            </p>

                                            <p className="v2-booking-created-date">
                                                <i className="fas fa-clock"></i> 
                                                <strong>Ngày đặt:</strong> 
                                                {new Date(booking.created_at || booking.booking_date).toLocaleDateString('vi-VN')}
                                            </p>
                                            <p><i className="fas fa-calendar"></i> <strong>Ngày khởi hành:</strong> {new Date(booking.departure_date).toLocaleDateString('vi-VN')}</p>
                                            <p><i className="fas fa-user"></i> <strong>Người liên hệ:</strong> {booking.contact_name}</p>
                                            <p><i className="fas fa-phone"></i> <strong>SĐT:</strong> {booking.contact_phone}</p>
                                            <p><i className="fas fa-envelope"></i> <strong>Email:</strong> {booking.contact_email}</p>
                                            
                                            {isPendingCancellation && (
                                                <div className="v2-pending-cancellation-notice">
                                                    <i className="fas fa-clock"></i>
                                                    <span>Yêu cầu hủy tour đang chờ admin xem xét (24-48h)</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="v2-booking-card-actions">
                                            ...
                                            {isPendingCancellation && (
                                                <button disabled className="v2-btn v2-btn--disabled">
                                                    <i className="fas fa-clock"></i> Đang chờ admin duyệt
                                                </button>
                                            )}
                                        </div>
                                    </div>
