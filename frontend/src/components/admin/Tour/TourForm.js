...
const TourForm = ({ formData, setFormData, onSubmit, onCancel, isSubmitting }) => {
  ...
  const updateScheduleBasedOnDates = useCallback((startDate, endDate, diffDays) => {
  ...
  useEffect(() => {
    if (formData.departure_date && formData.return_date) {
      ...
      if (endDate >= startDate) {
        ...
        if (!formData.schedule || formData.schedule.length === 0 || formData.auto_update_schedule) {
          updateScheduleBasedOnDates(startDate, endDate, diffDays);
        }
      }
    }
  }, [formData.departure_date, formData.return_date, formData.schedule, formData.auto_update_schedule, updateScheduleBasedOnDates]);
  ...
  const handleLocationChange = (scheduleIndex, selectedOptions) => {
    const newSchedule = [...formData.schedule];
    const locationIds = Array.from(selectedOptions).map(option => option.value);
    newSchedule[scheduleIndex] = {
      ...newSchedule[scheduleIndex],
    ...
    // Đồng bộ lại selected_location_ids từ tất cả locations trong schedule
    syncLocationsToSelectedIds();
  };
  ...
  const updateLocationsInSchedule = (locationIds) => {
    ...
    const updatedSchedule = formData.schedule.map((day, index) => {
      ...
      const endIdx = Math.min(startIdx + locationsPerDay, normalizedLocationIds.length);
  ...
  const handleDestinationsSelect = (e) => {
    ...
    if (selectedLocations.length > 0) {
      ...
      const locationIdsAsNumbers = selectedLocationIds.map(id => parseInt(id, 10));
  ...
  const syncLocationsToSelectedIds = () => {
    ...
    formData.schedule.forEach(day => {
      if (day.locations && Array.isArray(day.locations)) {
        day.locations.forEach(locId => {
          const numericId = typeof locId === 'string' ? parseInt(locId, 10) : locId;
    ...
    if (JSON.stringify([...locationIdsArray].sort()) !== JSON.stringify([...currentSelectedIds].sort())) {
      setFormData(prev => ({
        ...prev,
    ...
    return (
      <div className="selected-day-locations">
        ...
        <ul className="location-list">
          {day.locations.map((locId, idx) => {
            const location = availableLocations.find(loc => loc.id.toString() === locId.toString());
            return (
              <li key={idx} className="location-item">
                {location ? (location.title || location.name) : `ID: ${locId}`}
              </li>
            );
          })}
        </ul>
  ...
  const handleFormSubmit = (e) => {
    ...
    if (!formData.destination || !formData.departure_from || !formData.duration) {
    ...
    processedData.selected_location_ids = Array.isArray(processedData.selected_location_ids)
  ...
  return (
    <form onSubmit={handleFormSubmit} className="tour-form">
      ...
      <div className="tour-form-section">
        ...
        {isLoadingLocations ? (
          <div className="loading-locations">Đang tải danh sách địa điểm...</div>
        ) : (
        ...
        <div className="tour-form-group">
          ...
          <input
            ...
            required
          />
          <small>Thời gian được tính tự động từ ngày khởi hành và ngày kết thúc</small>
        </div>
        ...
        <div className="tour-form-checkbox">
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.checked && formData.departure_date && formData.return_date) {
                const startDate = new Date(formData.departure_date);
                const endDate = new Date(formData.return_date);
                const diffTime = endDate.getTime() - startDate.getTime();
                const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1;
              ...
              {day.activities.map((activity, activityIndex) => (
                <div key={activityIndex} className="tour-array-input">
                  <input
                    type="text"
                    value={activity}
                    onChange={(e) => handleActivityChange(scheduleIndex, activityIndex, e.target.value)}
                    placeholder="Mô tả hoạt động (VD: 08:00 - Check-in khách sạn)"
                  />
                  <button
                    type="button"
                    onClick={() => handleRemoveActivity(scheduleIndex, activityIndex)}
