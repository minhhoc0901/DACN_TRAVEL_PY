...
class Booking {
 ...
 static async findByPk(id) {
  ...
  /**
   * HÀM 2: HỦY BOOKING ĐÃ THANH TOÁN - VỚI CHÍNH SÁCH HOÀN TIỀN
   * Chính sách:
   * - Hủy trước 7 ngày: Hoàn 80%
   * - Hủy từ 3-7 ngày: Hoàn 50%
  ...
  static async cancelPaidBooking(bookingId, userId, refundMethod = 'credit') {
    ...
    try {
      ...
      // Lấy thông tin booking với departure_date và payment
      ...
      if (booking.payment_status !== 'success' || !booking.paid_amount) {
      ...
      // Kiểm tra đã qua ngày khởi hành chưa
      ...
      let refundAmount = 0;
      let refundReason = '';
      let refundStatus = 'rejected'; // Mặc định không hoàn

      if (daysUntilDeparture >= 7) {
        refundPercent = 80;
        refundReason = `Hủy trước ${daysUntilDeparture} ngày, được hoàn 80%`;
        refundStatus = 'pending';
      } else if (daysUntilDeparture >= 3) {
        refundPercent = 50;
        refundReason = `Hủy trước ${daysUntilDeparture} ngày, được hoàn 50%`;
        refundStatus = 'pending';
      } else {
        refundPercent = 0;
        refundReason = `Hủy trước ${daysUntilDeparture} ngày (dưới 3 ngày), không được hoàn tiền theo chính sách`;
        refundStatus = 'rejected';
      }

      refundAmount = Math.floor(booking.paid_amount * (refundPercent / 100));

      let refundId = null;
      let processingFee = 0;
      let finalRefundAmount = refundAmount;

      // XỬ LÝ THEO HÌNH THỨC HOÀN TIỀN
      if (refundMethod === 'credit') {
        // HOÀN VÀO CREDIT - NGAY LẬP TỨC
        refundStatus = 'approved'; // Tự động duyệt
        
        // Tạo bản ghi refund
        const [refundResult] = await connection.execute(`
            INSERT INTO refunds 
            (booking_id, refund_amount, refund_percent, refund_reason, refund_status, refund_method, processing_fee, created_at, approved_at)
            VALUES (?, ?, ?, ?, 'approved', 'credit', 0, NOW(), NOW())
        ...
        await UserCredit.addCredit(
            ...
            refundAmount,
            refundId,
            `Hoàn tiền từ booking #${bookingId}`
        );

        console.log(`[cancelPaidBooking] ✅ Credited ${refundAmount} VNĐ to user ${userId}`);

      } else if (refundMethod === 'bank_transfer') {
        // HOÀN QUA NGÂN HÀNG - CẦN ADMIN DUYỆT
        processingFee = Math.ceil(refundAmount * 0.02); // Phí 2%
        finalRefundAmount = refundAmount - processingFee;
        refundStatus = 'pending';
        
        const [refundResult] = await connection.execute(`
            INSERT INTO refunds 
            (booking_id, refund_amount, refund_percent, refund_reason, refund_status, refund_method, processing_fee, created_at)
            VALUES (?, ?, ?, ?, 'pending', 'bank_transfer', ?, NOW())
        `, [bookingId, finalRefundAmount, refundPercent, refundReason, processingFee]);
        
        refundId = refundResult.insertId;

        console.log(`[cancelPaidBooking] ⏳ Bank transfer refund pending admin approval`);
      }
