...
exports.cancelMyBooking = async (req, res) => {
    try {
        ...
        // GỬI THÔNG BÁO
        const io = req.app.get('io');
        await NotificationService.notifyBookingCancelled(io, userId, bookingId);
...
exports.cancelPaidBooking = async (req, res) => {
    try {
        const bookingId = req.params.id;
        const userId = req.user.id;
        const { refundMethod = 'credit' } = req.body; // LẤY TỪ REQUEST BODY

        console.log(`[CANCEL PAID BOOKING] User ${userId} cancelling booking ${bookingId} with method: ${refundMethod}`);

        // TRUYỀN ĐẦY ĐỦ 3 THAM SỐ
        const result = await Booking.cancelPaidBooking(bookingId, userId, refundMethod);

        // GỬI THÔNG BÁO REALTIME
        const io = req.app.get('io');

        if (result.refundMethod === 'credit' && result.refundStatus === 'approved') {
            // HOÀN VÀO CREDIT - THÔNG BÁO NGAY LẬP TỨC
        ...
        } else if (result.refundMethod === 'bank_transfer' && result.refundStatus === 'pending') {
            // HOÀN QUA NGÂN HÀNG - THÔNG BÁO CHỜ DUYỆT
            ...
            // THÔNG BÁO CHO ADMIN CÓ YÊU CẦU HOÀN TIỀN MỚI
            const adminIds = await NotificationService.getAdminIds();
            await NotificationService.notifyAdminNewRefund(
                io,
                adminIds,
                bookingId,
                result.bookingDetails.tourName,
        ...
        } else if (result.refundStatus === 'rejected') {
            // KHÔNG ĐƯỢC HOÀN TIỀN - THÔNG BÁO LÝ DO
        ...
        res.json({
            success: true,
            message: 'Hủy booking đã thanh toán thành công',
            ...result // Trả về toàn bộ kết quả từ Model
        });
